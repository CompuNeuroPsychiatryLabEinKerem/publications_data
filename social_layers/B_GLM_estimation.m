...
% 1. Open each *_IIHC_MNI.vmr file in BrainVoyager.
% 2. White matter mask: Under segmentation tools (under 3D 
% volume tools), choose first intensity values of min=150, max=255, grow 
% region, options, define VOI, name: WM_mask.
% 3. CSF mask: choose intensity values of min=0, max=10, bounding box: x
% between 90-160, y between 100-200, z between 100-150. Grow region,
% options, define VOI, name: CSF_mask.
% 4. Save both VOIs as "WM_CSF_vois.voi".
...
for s = 1:length(subject_names)
    ...
    for i=1:length(vtc_files)
        ...
        prt_filename = [vtc_filename(1:strfind(vtc_filename, '_SCCTBL_3DMCTS')-1) '_PRT.prt'];
        ...
        % Defining the design matrix parameters with the five predictors of scales
        vmr.ClearDesignMatrix;
        vmr.AddPredictor('scale1'); vmr.AddPredictor('scale2'); vmr.AddPredictor('scale3'); 
        vmr.AddPredictor('scale4'); vmr.AddPredictor('scale5');
        vmr.SetPredictorValuesFromCondition('scale1', 'scale1', 1.0); vmr.SetPredictorValuesFromCondition('scale2', 'scale2', 1.0); vmr.SetPredictorValuesFromCondition('scale3', 'scale3', 1.0);
        vmr.SetPredictorValuesFromCondition('scale4', 'scale4', 1.0); vmr.SetPredictorValuesFromCondition('scale5', 'scale5', 1.0);
        vmr.ApplyHemodynamicResponseFunctionToPredictor('scale1'); vmr.ApplyHemodynamicResponseFunctionToPredictor('scale2'); vmr.ApplyHemodynamicResponseFunctionToPredictor('scale3');
        vmr.ApplyHemodynamicResponseFunctionToPredictor('scale4'); vmr.ApplyHemodynamicResponseFunctionToPredictor('scale5');
...
mkdir(subjects_group_analysis_dir);
...
for s=1:length(subject_names)
    ...
    for i=1:length(vtc_files)
        ...
        [~,vtc_filename,~] = fileparts(vtc_files{i});
        prt_filename = [vtc_filename(1:strfind(vtc_filename, '_SCCTBL_3DMCTS')-1) '_PRT.prt'];
        prt_file = fullfile(output_dir, prt_filename);
        
        curr_vtc.NameOfLinkedPRT = prt_file;
