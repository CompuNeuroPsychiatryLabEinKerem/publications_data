...
% for anatomical data.
subjects_dicom_dir = 'F:\Scales_of_space_project\DICOMS\';    % the directory with the DICOM files for each subject, arranged in separate subject directories
...
functional_run_dirname_part = 'FUNC';
control_run_dirname_part = 'CONTROL';
...
for s=1:length(subject_names)
    ...
    for d=1:length(subfolders)
        ...
        elseif ~isempty(strfind(subfolders(d).name, functional_run_dirname_part))    % This is a directory with functional (BOLD) data, by the name of the subdirectory
            % defining parameters for conversion
            fileType = 'DICOM';
            bytesperpixel = 2;
            skipVols = 0;
            ...
            sizeX = 64; sizeY = 64;
            nrSlices = 46;
            mosaicSizeX = 448; mosaicSizeY = 448;
...
for s=1:length(subject_names)
    ...
    %     % Optional - boundary based registration - works less well in our experience
    %     for i=1:length(fmr_files)
    %         vmr = bvqx.OpenDocument(vmr_file);
    %         fmr_filename = fullfile(output_dir, fmr_files(i).name);
    %         vmr.CoregisterFMRToVMRUsingBBR(fmr_filename);
    %     end
